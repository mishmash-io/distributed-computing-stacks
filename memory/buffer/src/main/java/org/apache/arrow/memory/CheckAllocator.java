...
final class CheckAllocator {
  ...
  private static final String ALLOCATOR_PATH_FOREIGN =
      "org/apache/arrow/memory/foreign/DefaultAllocationManagerFactory.class";      
  ...
  static String check() {
    ...
    } else if (rootAllocator.getPath().contains("memory-foreign")
            || rootAllocator.getPath().contains("/org/apache/arrow/memory/foreign/")) {
          return "org.apache.arrow.memory.foreign.DefaultAllocationManagerFactory";
    } else {
  ...
  private static Set<URL> scanClasspath() {
    ...
    try {
      ...
      if (allocatorClassLoader == null) {
        ...
        if (!paths.hasMoreElements()) {
          paths = ClassLoader.getSystemResources(ALLOCATOR_PATH_FOREIGN);
        }
      } else {
        ...
        if (!paths.hasMoreElements()) {
          paths = allocatorClassLoader.getResources(ALLOCATOR_PATH_FOREIGN);
        }
      }
