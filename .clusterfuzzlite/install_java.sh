...
# Based on https://github.com/google/oss-fuzz/blob/master/infra/base-images/base-builder/install_java.sh

# Install OpenJDK 25 and trim its size by removing unused components. This enables using Jazzer's mutation framework.
cd /tmp
curl --silent -L -O https://github.com/adoptium/temurin25-binaries/releases/download/jdk-25.0.2+10/OpenJDK25U-jdk_x64_linux_hotspot_25.0.2_10.tar.gz && \
mkdir -p $JAVA_HOME
tar -xz --strip-components=1 -f OpenJDK25U-jdk_x64_linux_hotspot_25.0.2_10.tar.gz --directory $JAVA_HOME && \
rm -f OpenJDK25U-jdk_x64_linux_hotspot_25.0.2_10.tar.gz
...
mkdir -p $JAVA_17_HOME
tar -xz --strip-components=1 -f OpenJDK17U-jdk_x64_linux_hotspot_17.0.16_8.tar.gz --directory $JAVA_17_HOME && \
rm -f OpenJDK17U-jdk_x64_linux_hotspot_17.0.16_8.tar.gz
rm -rf $JAVA_17_HOME/jmods $JAVA_17_HOME/lib/src.zip
